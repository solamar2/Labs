...
nx=length(x(:));%Signal's length
nwin=length(win);
if (nwin == 1)
   len = win;%Window's length
else
   len = nwin;
end
if (nargin < 3)
...
nf = fix((nx-len+inc)/inc);%Num Of Sigments
f=zeros(nf,len);%row-sengment, len- sigment's length
indf= inc*(0:(nf-1)).';%the initial sample where the sigment starts
inds = (1:len);%sigment
