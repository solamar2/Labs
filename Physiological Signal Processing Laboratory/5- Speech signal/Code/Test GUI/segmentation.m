...
alpha = 0.95; WindowLength = 30*10^-3; Overlap = 50; 
[~,FramedSig] = PreProcess(signal,Fs,alpha,WindowLength,Overlap); %preprocessing
Idx = FindWordIdx(FramedSig,Fs,WindowLength,Overlap);

seg_ind=seg_ind+Idx(1,1);
n=min(6,length(seg_ind));seg_ind=seg_ind(1:n);
end